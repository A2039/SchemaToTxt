...
def add_sql_metadata(database_name, object_name, object_type):
    return f"""
USE [{database_name}]
GO

/****** Object:  {object_type} [dbo].[{object_name}]    Script Date: {time.strftime('%d-%m-%Y %H:%M:%S')} ******/
SET ANSI_NULLS ON
GO
...
def handle_locked_objects(locked_objects, database_name):
    if locked_objects:
        ...
        df = pd.DataFrame(locked_objects, columns=["Object Name", "Object Type"])
...
def read_and_create_txt_files(conn, database_name, object_type, query):
    ...
    try:
        ...
        main_folder = f"{database_name}"
        if not os.path.exists(main_folder):
            os.makedirs(main_folder)

        folder_name = os.path.join(main_folder, object_type.capitalize())
            ...
            for idx, row in enumerate(objects):
                ...
                try:
                    ...
                    if object_type == 'Tables':
                        # Fetch table definition by querying system views (information_schema or sys.columns)
                        cursor.execute(f"""
                        ...
                        # Create SQL content for the table definition
                        sql_content = add_sql_metadata(database_name, object_name, object_type)
                        ...
                        sql_content = add_sql_metadata(database_name, object_name, object_type) + object_definition
                    ...
                    if "No results" in str(e).lower() or "not a query" in str(e).lower():
                        locked_objects.append({"Object Name": object_name, "Object Type": object_type})
        ...
        print(f"❌ Error executing query for {object_type} in database {database_name}. Error: {e}")
        return 1
...
def main():
    ...
    object_queries = {
        ...
        'Functions': "SELECT name FROM sys.objects WHERE type IN ('FN', 'IF')",  # Scalar, Inline Table
        'Tables': "SELECT name FROM sys.tables",
        'Views': "SELECT name FROM sys.views",
        'Triggers': "SELECT name FROM sys.triggers",
        'Table_Valued_Functions': "SELECT name FROM sys.objects WHERE type = 'TF'" # Table-valued functions
    }
        ...
        for database_name in databases:
            try:
                ...
                for object_type, query in object_queries.items():
                    db_flag = read_and_create_txt_files(conn, database_name, object_type, query)
                    if db_flag == 1:
                        overall_flag = 1
